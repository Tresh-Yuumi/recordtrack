...
const EVENT_FIELDS = [
  ...
  'start_date', 'end_date', 'start_time', 'end_time', 'location', 'notes', 'image_urls',
]
...
async function execute(action, payload = {}) {
  ...
  if (action === 'createEvent') {
    const { data, error } = await supabaseAdmin.from('events')
      .insert({ ...pick(payload, EVENT_FIELDS), user_id: APP_USER_ID }).select('*').single()
  ...
  if (action === 'updateEvent') {
    const { data, error } = await supabaseAdmin.from('events')
      .update(pick(payload.updates || {}, EVENT_FIELDS))
  ...
  if (action === 'importData') {
    ...
    const events = backup.events.map((event) => ({ ...event, user_id: APP_USER_ID }))
